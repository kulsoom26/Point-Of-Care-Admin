  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response2 = await fetch(`${API_URL}api/admin/getTotalRadiologists`);
        const data2 = await response2.json();
        console.log(data2.number);
        setRadCount(data2.number);
        await setPercentage(parseFloat((RadCount / userCount) * 100).toFixed(2));
      } catch (error) {
  ...
  return (
    <StyledChartBox color={props.color}>
      <div className="boxInfo">
        ...
        <h1 style={{fontFamily:"Lato"}}>{RadCount}</h1>
      </div>
      <div className="chartInfo">
        ...
        <div className="texts">
          ...
          >
            {parseFloat((RadCount / userCount) * 100).toFixed(2)}%
          </span>
        </div>
      </div>
    </StyledChartBox>
